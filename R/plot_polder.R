...
plot_polder <- function(
  ...
  polder.z = NULL,
  h.lines = NULL,
  plot.title = paste('Ganglinien für Maßnahme: ', name),
  text.pos.x = 0.01,
  text.pos.y = NULL,
  date.break = '3 days',
  date.label = '%d.%m.%y',
  ...
  y2_min <- round(y2_shift/y2.scale, -1)
  ...
  if (tolower(param) == 'discharge'){
    ...
    if (isTRUE(w.canal)){
      q.in <- FALSE
      y2_min <- round(min(id_data$W_innen, na.rm = TRUE), -1)
      if (y2_min*y2.scale != y1_min) {
        y2_shift <- round(y2_shift - y2_min*y2.scale, -1)
      }
