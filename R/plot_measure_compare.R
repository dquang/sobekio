...
plot_measure_scenario <- function(
  ...
  # we compare two cases, so there must be more than one case and case %% 2 == 0
  # stopifnot(length(case.list) %% 2 == 0 & length(case.list) > 1)
  # currently support only two cases
  stopifnot(length(case.list) != 2)
  # checking input
  param = tolower(param)
  ...
  #     p_volume <- his_from_case(case.list = case.list,
  #                               sobek.project = sobek.project,
  #                               param = 'Volume',
